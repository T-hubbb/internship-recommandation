...
def recommend():
    ...
    if not internships:
        flash("No internships available in the database.", "danger")
        return render_template("upload.html")
    
    # Get recommendations
    recommendations = recommendInternship(student, internships, top_n=5)
    
    # Convert to friendly structure for Jinja
    results = []
    ...
    return render_template("recommendations.html", results=results, student=student)
